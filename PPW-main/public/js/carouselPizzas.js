...
//associationNumber è usato perchè così il numero di slot del carousel corrisponde a quello delle associazioni
function checkCurrRadio() {
    for (var i = 0; i < sessionStorage.getItem("associationNumber"); i++) {
...
function clickForward() {
    checkCurrRadio()
    buttonAppear("back")
    if (currRadio < sessionStorage.getItem("associationNumber")) {
        currRadio += 1;
...
function buttonDisappear(buttonId) {
    ...
    document.getElementById(buttonId).style.cursor = "transparent";
